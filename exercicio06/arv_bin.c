...
int esimo(Nodo* raiz){
    if (raiz == NULL)
        return 1;
    else if (raiz->esquerda != NULL && maior_elemento_no(raiz->esquerda) > raiz->info)
        return 0;

    else if (raiz->direita != NULL && menor_elemento_no(raiz->direita) < raiz->info)
        return 0;

    else if (!esimo(raiz->esquerda) || !esimo(raiz->direita))
        return 0;
